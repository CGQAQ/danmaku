<template>
    <section class="video-wrapper" ref="videoWrapper">
        <GlobalEvents @mousemove="onMousemove"/>
        <section class="danmaku-wrapper" :style="{width: wrapperWidth}">
            <video
                :src="videoURL"
                ref="video"
...
export default class VideoWrapper extends Vue {
    ...
    mounted() {
        this.danmakuMachine = new DanmakuMachine(
            undefined,
            undefined,
            this.danmakuURL
        );

        this.danmakuMachine.danmakuStream.subscribe(
            (danmaku: BilibiliDanmaku) => {
                ...
                if (mode !== DanmakuType.LTR && mode !== DanmakuType.RTL) {
                    ...
                    if (poolLen === 0) {
                        ...
                        const columnLength: number = this.normalDanmakuPool[
                            poolLen - 1
                        ].length;
                        const lastone: BilibiliDanmaku = this.normalDanmakuPool[
                            poolLen - 1
                        ][columnLength - 1];
                        if (
                            Math.abs(lastone.time - danmaku.time) <
                            this.threshold
                        ) {
        ...
        (this.$refs.video as HTMLVideoElement).addEventListener(
            "progress",
            (e: Event) => {
    ...
    onDanmakuShouldRemove(data: { id: string; mode: DanmakuType }) {
        ...
        if (data.mode !== DanmakuType.RTL && data.mode !== DanmakuType.LTR) {
            if (data.mode === DanmakuType.TOP) {
                const index = this.topDanmakuPool.findIndex(
                    d => d.rowID === data.id
                );
                this.topDanmakuPool.splice(index, 1);
            } else if (data.mode === DanmakuType.BOTTOM) {
                const index = this.bottomDanmakuPool.findIndex(
                    d => d.rowID === data.id
                );
            ...
            if (this.normalDanmakuPool[0].length > 1) {
                const index = this.normalDanmakuPool[0].findIndex(
                    d => d.rowID === data.id
                );
